<section class="section-hero bg-brown-shade-4">
	<div class="section-hero--wrapper">
		<?php
			...
			if ( $hero_image ) :
				echo wp_get_attachment_image( $hero_image, 'full', false, array( 'class' => 'section-hero--img w-full h-[100svh] object-cover', 'loading' => 'eager', 'fetchpriority' => 'high' ) );
			endif;
			$hero_image_sm = get_field( 'hero_section_hero_image_mobile' );
			if ( $hero_image_sm ) :
				echo wp_get_attachment_image( $hero_image_sm, 'full', false, array( 'class' => 'section-hero--img w-full h-[100svh] block lg:hidden max-h-none md:max-h-[75vh] lg:max-h-none', 'loading' => 'eager' ) );
			endif;
		?>
