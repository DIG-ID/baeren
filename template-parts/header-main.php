<header id="header-main" class="header-main absolute top-0 left-0 w-full z-[999] overflow-hidden" itemscope itemtype="https://schema.org/WebSite">

	<nav class="navbar relative overflow-hidden" aria-label="<?php esc_attr_e( 'Main menu', 'baeren' ); ?>">
		<div class="header-wrapper relative grid grid-cols-3 px-6 md:px-8 xl:px-16 py-6 md:py-8 xl:py-6 z-40 xl:z-auto">
			...
			<div class="col-span-1 hidden xl:flex justify-end items-center gap-x-8 order-3">
				...
				if ( $booking_url ) :
					?>
					<a href="<?php echo esc_url( $booking_url ); ?>" target="_blank" class="btn btn--header">
		...
		<div id="menu-wrapper" class="menu-wrapper w-full top-0 left-0 bottom-0 hidden fixed">
			...
			<div class="grid grid-cols-12 px-0 py-0 h-full">
				<div class="col-span-8 pl-48 pt-44 hidden xl:block bg-brown-shade-2">
					...
					wp_nav_menu(
						array(
							...
							'items_wrap'      => '<ul id="%1$s" class="%2$s">%3$s</ul>',
							'fallback_cb'     => '__return_false',
							//'walker'          => new Baeren_Walker_Nav_Menu(),
						)
					);
					?>
				</div>
				<div class="col-span-12 px-0 py-0 xl:pl-8 xl:pt-12 bg-brown-shade-2 block xl:hidden mobile__nav">
					<nav>
						<?php
						wp_nav_menu(
							array(
				...
				<div id="menu-secondary-col" class="col-span-4 bg-brown-shade-1 pt-44 text-center hidden xl:block">
