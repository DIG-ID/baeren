 ...
 */
function baeren_dequeue_unused_styles() {
...
add_action( 'wp_enqueue_scripts', 'baeren_dequeue_unused_styles', 100 );

/**
 * Dequeues the wp-embed script — embedding WP posts in external sites is rarely needed.
 */
function baeren_dequeue_unused_scripts() {
	wp_dequeue_script( 'wp-embed' );
}

add_action( 'wp_footer', 'baeren_dequeue_unused_scripts' );

/**
 * Removes unnecessary links and meta tags from wp_head.
 */
function baeren_clean_head() {
...
add_action( 'init', 'baeren_clean_head' );

/**
 * Disables the WordPress emoji detection script and styles.
 */
function baeren_disable_emojis() {
	...
	add_filter( 'tiny_mce_plugins', 'baeren_disable_emojis_tinymce' );
	add_filter( 'wp_resource_hints', 'baeren_disable_emojis_dns_prefetch', 10, 2 );
}

add_action( 'init', 'baeren_disable_emojis' );
 ...
 * @return array
 */
function baeren_disable_emojis_tinymce( $plugins ) {
	if ( is_array( $plugins ) ) {
 ...
 * @return array
 */
function baeren_disable_emojis_dns_prefetch( $urls, $relation_type ) {
 ...
 */
function baeren_disable_self_pingbacks( &$links ) {
...
add_action( 'pre_ping', 'baeren_disable_self_pingbacks' );
